...
<head>
    ...
    *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
    ...
    .filter-row {
        ...
        opacity:0; animation:fadeUp 0.5s ease 0.25s forwards;
    }
    .ftab {
        padding:7px 16px; border-radius:20px;
        font-size:0.74rem; font-weight:700; letter-spacing:0.05em; text-transform:uppercase;
        border:1px solid var(--border2); background:transparent;
        color:var(--text3); cursor:pointer; transition:all 0.2s; font-family:'Outfit',sans-serif;
    ...
    .ftab .cbadge {
        ...
        font-size:0.6rem; font-weight:800; background:rgba(255,255,255,0.2);
    }
    ...
    .bk-img {
        width:190px; flex-shrink:0; overflow:hidden; position:relative;
    }
    .bk-img img {
    ...
    .empty-icon {
        ...
        display:flex; align-items:center; justify-content:center;
        font-size:1.8rem; color:var(--text3); margin-bottom:20px;
    }
    ...
    .explore-btn {
        ...
        text-decoration:none; transition:all 0.22s; box-shadow:0 4px 14px var(--accentglow);
    }
    ...
    @media(max-width:960px) {
        ...
        .bk-img { width:140px; }
        .bk-details { grid-template-columns:1fr 1fr; }
    }
    @media(max-width:640px) {
        .main { margin-left:0; width:100%; }
        .sidebar { display:none; }
        .bk-card { flex-direction:column; }
        .bk-img { width:100%; height:180px; }
        .bk-actions { padding:0 18px 18px; flex-direction:row; }
        .stats-row { grid-template-columns:1fr 1fr; }
    }
    </style>
</head>
    ...
    <div class="body">
        ...
        <div class="stats-row">
            <div class="sc">
                ...
                <div><div class="sc-num"><?php echo $total_rows; ?></div><div class="sc-lbl">Total</div></div>
            </div>
            <div class="sc">
                <div class="sc-icon" style="background:var(--greenbg);color:var(--green);"><i class="fa fa-circle-check"></i></div>
                <div><div class="sc-num"><?php echo $confirmed; ?></div><div class="sc-lbl">Confirmed</div></div>
            </div>
            <div class="sc">
                <div class="sc-icon" style="background:var(--amberbg);color:var(--amber);"><i class="fa fa-clock"></i></div>
                <div><div class="sc-num"><?php echo $pending; ?></div><div class="sc-lbl">Pending</div></div>
            </div>
            <div class="sc">
                <div class="sc-icon" style="background:var(--redbg);color:var(--red);"><i class="fa fa-ban"></i></div>
                <div><div class="sc-num"><?php echo $cancelled; ?></div><div class="sc-lbl">Cancelled</div></div>
            </div>
        </div>

        <!-- FILTER TABS -->
        <div class="filter-row">
            <button class="ftab active" onclick="filterBookings('all',this)">
                All <span class="cbadge"><?php echo $total_rows; ?></span>
            </button>
            <button class="ftab" onclick="filterBookings('confirmed',this)">
                Confirmed <span class="cbadge"><?php echo $confirmed; ?></span>
            </button>
            <button class="ftab" onclick="filterBookings('pending',this)">
                Pending <span class="cbadge"><?php echo $pending; ?></span>
            </button>
            <button class="ftab" onclick="filterBookings('cancelled',this)">
                Cancelled <span class="cbadge"><?php echo $cancelled; ?></span>
            </button>
        </div>
        ...
        <?php if (count($all_bookings) > 0):
            ...
            foreach ($all_bookings as $row):
                $s = intval($row['status']);
                if ($s === 1)     { $sc='confirmed'; $st='Confirmed'; $bc='sbadge-confirmed'; }
                elseif ($s === 2) { $sc='cancelled'; $st='Cancelled'; $bc='sbadge-cancelled'; }
                else              { $sc='pending';   $st='Pending';   $bc='sbadge-pending'; }

                $days  = max(1, ceil((strtotime($row['to_date']) - strtotime($row['from_date'])) / 86400));
                $total = $days * $row['price_per_day'];
                $img   = !empty($row['car_image'])
                    ? "../admin/img/vehicleimages/" . htmlspecialchars($row['car_image'])
                    : "https://images.unsplash.com/photo-1503376780353-7e6692767b70?w=400";
        ?>
        <div class="bk-card bk-<?php echo $sc; ?>" data-status="<?php echo $sc; ?>"
            ...
            <div class="bk-body">
                <div class="bk-top">
                    ...
                    <span class="sbadge <?php echo $bc; ?>"><?php echo $st; ?></span>
                </div>
                <div class="bk-details">
                    <div>
                        <div class="bk-dl">Pick-up</div>
                        <div class="bk-dv"><i class="fa fa-calendar"></i><?php echo date('d M Y', strtotime($row['from_date'])); ?></div>
                    </div>
                    <div>
                        <div class="bk-dl">Return</div>
                        <div class="bk-dv"><i class="fa fa-calendar-check"></i><?php echo date('d M Y', strtotime($row['to_date'])); ?></div>
                    </div>
                    <div>
                        <div class="bk-dl">Duration</div>
                        <div class="bk-dv"><i class="fa fa-clock"></i><?php echo $days; ?> day<?php echo $days!=1?'s':''; ?></div>
                    </div>
                    <div>
                        <div class="bk-dl">Total Cost</div>
                        <div class="bk-dv total"><i class="fa fa-tag"></i>LKR <?php echo number_format($total); ?></div>
                    </div>
                </div>
            </div>
            ...
            <div class="bk-actions">
                ...
                <a href="cancel_booking.php?id=<?php echo $row['id']; ?>"
                   onclick="return confirm('Cancel booking #BK-<?php echo str_pad($row['id'],5,'0',STR_PAD_LEFT); ?>?')"
                   class="act-cancel">
...
<script>
    ...
    // Theme — cfyTheme key (shared with login & dashboard)

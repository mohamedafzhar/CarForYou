...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $res = mysqli_query($conn, "SELECT id, full_name FROM users WHERE email='$email' LIMIT 1");
    ...
    if (!$user) {
        ...
        mysqli_query($conn, "SET time_zone = '+05:30'"); // Sri Lanka = UTC+5:30

        // Store token in DB (create this table — see below)
        mysqli_query($conn, "DELETE FROM password_resets WHERE email='$email'"); // clear old
        ...
        try {
            ...
            $mail->SMTPAuth = true;
            $mail->Username = 'amafzhar@gmail.com';       // ← your Gmail
            $mail->Password = 'qwak olmr mzhm urvq';  // ← 16-char App Password
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
            $mail->Port = 587;

            $mail->setFrom('amafzhar@gmail.com', 'carforyou');
    ...
    <div style='font-family:Outfit,sans-serif;max-width:520px;margin:auto;background:#0f1319;color:#f0f2f8;padding:36px;border-radius:16px;border:1px solid rgba(255,255,255,0.08)'>
        ...
        <p style='color:#8892a4;font-size:14px;line-height:1.6;margin-bottom:20px'>
            ...
            We received a request to reset your CarForYou password. Click the button below — this link is valid for <strong style='color:#fbbf24'>1 hour</strong>.
        </p>
        <div style='text-align:center;margin:28px 0'>
            <a href='$reset_link'
               style='display:inline-block;padding:13px 28px;background:linear-gradient(135deg,#00d4ff,#0090ff);color:#fff;border-radius:10px;text-decoration:none;font-weight:700;font-size:14px;letter-spacing:0.05em'>
                Reset My Password
            </a>
        </div>
        <p style='color:#44505e;font-size:12px;line-height:1.6;border-top:1px solid rgba(255,255,255,0.06);padding-top:16px;margin-top:8px'>
            ...
            // Uncomment below to debug SMTP errors:
            ...
            <div class="card-body">
                <?php if ($success_msg): ?>
                    <div class="success-state">
                        ...
                        <p><?php echo $success_msg; ?><br><br>
                            The link will expire in <strong style="color:var(--accent)">1 hour</strong>. Check your spam
                            folder if you don't see it.</p>
                    </div>
